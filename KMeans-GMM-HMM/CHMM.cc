...
CHMM::CHMM(int stateNum, int dimNum, int mixNum)
{
    ...
    for (int i = 0; i < m_stateNum; i++)
    {
        ...
        {
            m_stateTran[i][j] = 0.0 / (m_stateNum + 1);
        }
    }
}
...
double CHMM::Decode(vector<double*>& seq, vector<int>& state)
{
    ...
    // Add final Prob
    for ( i = 0; i < m_stateNum; i++) {
         currLogP[i] += LogProb(m_stateTran[i][m_stateNum]);
    }
    
    // Termination
    int finalState = 0;
    double prob = -BIG_DOUBLE;
    for ( i = 0; i < m_stateNum; i++)
    {   
        if (currLogP[i] > prob)
        {
    ...
    if (finalState != m_stateNum-1) {
        std::cout << "wrong decode" << std::endl;
        std::cout << "sequence len: " << size;
        std::cout << ";final state: " << finalState << "; num of state: " << m_stateNum << std::endl;;
    }
    // Decode
...
void CHMM::Init(const std::vector<infra::matrix> instances) {
    int i,j,k;
    int size = 0;
    int dim = 0;
    size = instances.size();
    ...
    for ( i = 0; i < m_stateNum; i++)
    {
        ...
        if(i == 0) {
            ...
            m_stateInit[i] = 0.0 / float(m_stateNum-1);
        }
        // The transition probabilities
        for ( j = 0; j < m_stateNum; j++)
        {
    ...
    for ( i = 0; i < m_stateNum; i++) {
        int seq_size = gaussseq[i].size();
        double *single_gmm_data;
        single_gmm_data = new double [seq_size * dim];
        for (j = 0; j < seq_size; j++) {
            for ( k = 0; k < dim; k++) {
        ...
        delete[] single_gmm_data;
        gaussseq[i].clear();
    }
    delete[] gaussseq;
}
    
void CHMM::Train(const std::vector<infra::matrix> instances, bool tune_trans) {
    Init(instances);
    int i,j,k;
    ...
    while (loop)
    {
        ...
        for ( i = 0; i < size; i++) {
            int seq_size = instances[i].height();
            for ( j = 0; j < seq_size; j++) {
                double* x = new double[dim];
                for ( k = 0; k < dim; k++) {
                    x[k] = instances[i](j, k);
            ...
            for (j = 0; j < seq_size; j++) {
                gaussseq[state[j]].push_back(seq[j]);
            }
        ...
        // here we tune the transition prob
        if (tune_trans) {
            for ( j = 0; j < m_stateNum; j++) {
...
void CHMM::LengthNormalization() {
    for(int i=0; i < m_stateNum; i++) {
        m_stateModel[i]->LengthNormalization();
    }
...
double CHMM::LogProb(double p) {
    return (p > 5e-300) ? log(p) : -BIG_DOUBLE;
}
